...
function kalshiReq(method, p, body=null) {
  return request({
    ...
    headers: { 'Content-Type':'application/json', 'Authorization':'Bearer '+KALSHI_KEY }
  }, body);
}
...
function kellySize(price, cash, conf, score) {
...
async function run() {
  ...
  try {
    ...
    if(r.status===200&&r.body.markets?.length) {
      ...
      addLog('Rex',`Kalshi returned ${r.status}`,'lt-warn');
    }
  ...
  if(expPct<55) {
    ...
    addLog('Axel','Exposure cap 55% — holding','lt-warn');
  }
