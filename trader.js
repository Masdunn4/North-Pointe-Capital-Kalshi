...
function isShort(m) {
  /* 7-day window (168h) — captures more real Kalshi markets as short-term.
     Most Kalshi markets that close within a week count as short-term for
     our Kelly sizing and Nova diversification purposes. */
  const t=new Date(m.close_time||m.expiration_time||Date.now()+999*86400000);
  return (t-Date.now())/3600000<=168;
}
...
function scoreMarket(m) {
  ...
  /* Lowered thresholds — real Kalshi markets score 5-7 typically */
  const conf=score>=8?'high':score>=6?'med':score>=4?'low':'skip';
  if(conf==='skip') return null;
  const rec=score>=8?'Strong BUY':score>=6?'BUY':'Watch';
...
async function run() {
  ...
  for(const m of STATE.markets) {
    ...
    /* Allow Watch bets through — Nova ranks by score, Axel filters by conf */
  ...
  /* 4. NOVA — diversify, targeting 70% short-term / 30% long-term ──
     Short-term buckets get filled first since that's our priority target.
     If there aren't enough short candidates some ticks, long-term fills
     the gap so we don't leave approved slots empty — but short always
     gets first pick when both qualify. */
  const B={
  ...
  const TOTAL_SLOTS=8;
  const SHORT_TARGET=Math.round(TOTAL_SLOTS*0.70); /* 6 of 8 slots */
  const usedCats=new Set(),seen=new Set(),approved=[];

  /* Pass 1: fill short-term slots first (sY/sN alternating for YES/NO balance) */
  const shortOrder=[B.sY,B.sN];
  let shortRound=0;
  while(approved.filter(x=>x.termType==='short').length<SHORT_TARGET && shortRound<20) {
    shortRound++;
    let addedAny=false;
    for(const pool of shortOrder) {
      if(approved.filter(x=>x.termType==='short').length>=SHORT_TARGET) break;
    ...
    if(!addedAny) break; /* no more short candidates available this tick */
  }

  /* Pass 2: fill remaining slots with long-term (only if short ran dry) */
  const longOrder=[B.lY,B.lN];
  let longRound=0;
  while(approved.length<TOTAL_SLOTS && longRound<20) {
    longRound++;
    let addedAny=false;
    for(const pool of longOrder) {
      if(approved.length>=TOTAL_SLOTS) break;
      let pick=pool.find(x=>!seen.has(x.m.ticker)&&!usedCats.has(x.m.category));
      if(!pick) pick=pool.find(x=>!seen.has(x.m.ticker));
      if(!pick) continue;
      seen.add(pick.m.ticker);usedCats.add(pick.m.category);approved.push(pick);
  ...
  const shortCount=approved.filter(x=>x.termType==='short').length;
  const longCount=approved.length-shortCount;
  addLog('Nova',`Diversified: ${shortCount} short (${approved.length?Math.round(shortCount/approved.length*100):0}%) · ${longCount} long — target 70% short`);
  ...
  /* Accept high, med, AND low confidence — with real markets
     low-conf bets with big payouts (NO at 10-20¢) are often the
     most profitable. Score still determines sizing priority. */
  const eligible=approved.filter(x=>x.a.conf!=='skip');
  const toPlace=[];
  ...
  /* 10% cash floor — keep $20 reserve on $200, deploy the rest.
     Previously 30% ($60) was stopping Axel too early. */
  const cashFloor = STATE.startCash * 0.10;
  if(STATE.cash < cashFloor) {
    ...
    for(const x of eligible) {
      if(toPlace.length>=8) break;
      if(STATE.cash - toPlace.reduce((s,p)=>s+(p._kSize||0),0) < cashFloor) break;
      /* Cap: up to 6 short-term, up to 2 long-term per cycle — matches
         the 70%/30% short/long target Nova diversified for. Side (YES/NO)
         still capped at 4 each to keep some balance there. */
      if((x.termType==='short'&&counts.short>=6)||(x.termType==='long'&&counts.long>=2)) continue;
      if(counts[x.a.side]>=4) continue;
  ...
  for(const pos of STATE.positions) {
    ...
    /* Paper simulation fallback — if Kalshi hasnt settled yet,
       simulate settlement based on implied probability + our edge.
       Short bets eligible after 90 ticks (~22.5 min at 15-min runs = ~1-2 runs)
       Long bets eligible after 4 ticks (~1 hour of runs).
       This keeps capital cycling instead of locking up for weeks. */
    if(!toClose.find(x=>x.pos.id===pos.id)) {
      const minTicks = pos.shortTerm ? 2 : 4;
      const settleProb = pos.ticksOpen < minTicks ? 0
                       : pos.shortTerm ? Math.min((pos.ticksOpen-2)*0.15, 0.40)
                       : Math.min((pos.ticksOpen-4)*0.05, 0.15);
      if(settleProb > 0 && Math.random() < settleProb) {
        const edgeB=(pos.conf==='high'?0.10:pos.conf==='med'?0.06:0.03);
        const winProb=Math.min((pos.entryPrice||0.7)+edgeB, 0.96);
        const won=Math.random()<winProb;
        const payout=won?parseFloat((pos.size/pos.entryPrice).toFixed(2)):0;
        const pnl=parseFloat((payout-pos.size).toFixed(2));
        toClose.push({pos,won,pnl,result:won?'yes':'no',paper:true});
        addLog('Knox',`[PAPER SETTLE] ${pos.ticker} tick:${pos.ticksOpen} → ${won?'WIN ✓':'LOSS ✗'} (winP:${(winProb*100).toFixed(0)}%)`,won?'lt-trade':'lt-err');
      } else {
        addLog('Knox',`${pos.ticker} [${pos.shortTerm?'short':'long'}] tick:${pos.ticksOpen} · waiting · settle chance:${(settleProb*100).toFixed(0)}%`);
      }
    }
    await sleep(200);
  }
